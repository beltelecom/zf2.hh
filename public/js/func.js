...
var button = Ext.create('Ext.button.Button', { 
    ...
    Ext.Ajax.request({ 
                                                                             url: '/users/user-manager/index2',
                                                                             success: function(response, options){
                                                                             var objAjax1 = Ext.decode(response.responseText)
                                                                             //console.log(sss1.length); 
...
////////////////////////////////////////////////////////////        
    ...
    var childPanel2 = Ext.create('Ext.panel.Panel', {
     ...
     var store = Ext.create('Ext.data.JsonStore', {     // определение хранилища для удаленного источника данных
            fields: [ 'id','name'],   // поля записей. каждая запись содержит - название городва и телефон
                           // количество считываемх зАраз записей
            proxy: {                    // описание proxy-объекта, кторый будет запрашивать сервер
            type: 'ajax',           // тип прокси = Ajax
            url: '/users/user-manager/index1',         // адрес удаленного источника данных
           reader: {
...
//console.log(store.count()); 
    ...
    var ii = 0; var sss2 = new Array()
    Ext.Ajax.request({ 
                                                                             url: '/users/user-manager/index3',
                                                                             success: function(response, options){
                                                                             var objAjax1 = Ext.decode(response.responseText)
                                                                             //console.log(sss2.length); 
                                                                             for (jj=0; jj<objAjax1.length; jj++){
                                                                                //console.log(sss2.length)                                                                     
                                                                             sss2[ii] = {
                                                                             xtype: 'checkbox',
